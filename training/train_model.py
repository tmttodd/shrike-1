#!/usr/bin/env python3
"""Generic BERT-family classifier trainer. Works with any HF model."""
...
print(f"{MODEL_NAME}: {len(real)} real, {len(synth)} synth, {num_labels} classes", file=sys.stderr)
...
print(f"  Params: {params:.0f}M", file=sys.stderr)
...
json.dump({"train": result.metrics, "eval": ev, "model_name": MODEL_NAME, "params_M": params,
           "num_classes": num_labels, "epochs": EPOCHS, "lr": LR, "batch": BATCH}, open(out / "training_metrics.json", "w"), indent=2)
...
below95 = sum(1 for k, v in report.items() if isinstance(v, dict) and "f1-score" in v and v["f1-score"] < 0.95 and k not in ("macro avg", "weighted avg"))
total_cls = sum(1 for k, v in report.items() if isinstance(v, dict) and "f1-score" in v and k not in ("macro avg", "weighted avg"))
print(f"  Weighted F1: {wf1:.4f}, Classes <95%: {below95}/{total_cls}", file=sys.stderr)
print(f"SUMMARY|{MODEL_NAME}|{acc:.4f}|{wf1:.4f}|{params:.0f}M|{below95}/{total_cls}", file=sys.stderr)
