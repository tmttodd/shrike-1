...
def count_pattern_fields(confidence: dict) -> int:
    """Count fields extracted by high-confidence methods."""
    return sum(1 for v in confidence.values() if v in ("pattern", "alias", "ner"))
...
def run_report(ground_truth_path: str, seed: int = 42,
               tiered: bool = False, api_base: str = "",
               model: str = "shrike-extractor") -> dict:
    """Run the quality report and return results as a dict.

    Args:
        tiered: If True, use TieredExtractor (includes LLM Tier 2/3 + fingerprint cache).
        api_base: API base URL for LLM (required if tiered=True).
        model: Model name for LLM extraction.
    """
    random.seed(seed)
    ...
    extractor: Any
    if tiered and api_base:
        from shrike.extractor.tiered_extractor import TieredExtractor
        extractor = TieredExtractor(api_base=api_base, model=model)
    else:
    ...
    for r in test:
        ...
        if tiered and api_base:
            result, tier = extractor.extract(
                r["raw_log"], fmt, r["class_uid"], r.get("class_name", ""))
            tier_counts[tier] += 1
        else:
        ...
        if result is None:
            ...
            quality = classify_quality(result.event, result.confidence or {})
    ...
    # Save fingerprint cache if tiered
    if tiered and api_base and hasattr(extractor, 'save_cache'):
    ...
    return {
        ...
        "tier_counts": dict(tier_counts) if tiered else {},
    }
...
def main():
    ...
    parser.add_argument("--tiered", action="store_true",
                        help="Use TieredExtractor with LLM fallback (requires --api-base)")
    parser.add_argument("--api-base", type=str, default="",
                        help="LLM API base URL for tiered extraction")
    parser.add_argument("--model", type=str, default="shrike-extractor",
                        help="Model name for LLM extraction")
    args = parser.parse_args()

    report = run_report(args.ground_truth, args.seed,
                       tiered=args.tiered, api_base=args.api_base,
                       model=args.model)
